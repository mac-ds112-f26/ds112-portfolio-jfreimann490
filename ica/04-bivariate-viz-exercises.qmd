...
I would wonder why it is so left skewed.
```{r}
...
#Changes the shape of the data point.
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16)) +
  geom_point(shape = 3)
```
...
#Color is the right option here. 
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16)) +
...
The support was roughly the same for both years and is a positive stong relationship. 
...
#Summary: Data is skewed right. There is a negative relationship between median rent and republican percentage in 2020. 
...
#Summary: There is a moderately positive relationship between the median age and republican percentage in 2020. 
```
...
It wouldn't be optimal because there is a categorical variable and you can't distinguish the points from each other. 
...
The majority of people living in blue counties historically, voted for the republican candidate. This number rises as you go to historically purple counties, and then even further for historically red counties.
...
```{r}
# The legend sucks and the axis aren't labeled enough.
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density()
```
...
#It allows you to color the plots in with the colors of your choice. 
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
...
#The transparency shifts up and down. 
...
#It's very hard to see all of the data and is super messy. 
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
...
I like the density plot the best. 
-   What is one pro of density plots relative to boxplots?
You can overlap much easier and represents the disribution more accurately. 
-   What is one con of density plots relative to boxplots?
The outliers are shown better in box plots than in density plots. 
...
#Historical is the category and the winner variable is the outcome separating the categories. 
ggplot(elections, aes(x = historical, fill = winner_20)) +
  geom_bar()
```
...
Con: You don't get a total count on the y-axis.
Pro: You get a percentage of the total for each category. 
#### Part b

What's your favorite bar plot from part and why?

My favorite is the stacked bar plot because it just makes sense to my eyes and I appreciate the total count still being there. 
...
ggplot(elections, aes(x = repub_pct_20, y = fct_reorder(state_abbr, repub_pct_20), fill = historical)) + 
  geom_density_ridges(alpha = 0.5) + 
  labs(x = "Republican Voting Percentage in 2020", y = "State") + 
  scale_fill_manual(values = c("blue", "purple", "red"))
...
MS has the most and HI has the least from the initial glance. 
-   What other interesting patterns do you notice?
The majority of the purple states have a lot of variability which makes sense. 
-   Does this plot prompt any other questions?
Which kind of states have the most influence on the outcome of an election?
